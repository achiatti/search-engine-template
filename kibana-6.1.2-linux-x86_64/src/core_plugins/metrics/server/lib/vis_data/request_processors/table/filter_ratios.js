    ...
    panel.series.forEach(column => {
      ...
      if (column.metrics.some(filter)) {
        column.metrics.filter(filter).forEach(metric => {
          _lodash2.default.set(doc, `${aggRoot}.timecolumn.aggs.${metric.id}-numerator.filter`, {
            query_string: { query: metric.numerator || '*', analyze_wildcard: true }
          });
          _lodash2.default.set(doc, `${aggRoot}.timecolumn.aggs.${metric.id}-denominator.filter`, {
          ...
          if (metric.metric_agg !== 'count' && _bucket_transform2.default[metric.metric_agg]) {
            ...
            _lodash2.default.set(doc, `${aggRoot}.timecolumn.aggs.${metric.id}-numerator.aggs`, aggBody);
            _lodash2.default.set(doc, `${aggBody}.timecolumn.aggs.${metric.id}-denominator.aggs`, aggBody);
            numeratorPath = `${metric.id}-numerator>metric`;
            denominatorPath = `${metric.id}-denominator>metric`;
          }

          _lodash2.default.set(doc, `${aggRoot}.timecolumn.aggs.${metric.id}`, {
            bucket_script: {
              buckets_path: {
